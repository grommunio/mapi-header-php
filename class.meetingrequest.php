...
class Meetingrequest {
	 ...
	 *
	 * $removeRequestOnResponse is opt-in; leaving the request mail alone is the
	 * default.
	 *
	 * @param mixed $store
	 * @param mixed $message
	 ...
	 * @param mixed $removeRequestOnResponse
	 */
	public function __construct(private $store, public $message, private $session = false, private $enableDirectBooking = true, private $removeRequestOnResponse = false) {
	...
	public function doAccept(bool $tentative, bool $sendresponse, bool $move, mixed $newProposedStartTime = false, mixed $newProposedEndTime = false, mixed $body = false, mixed $userAction = false, mixed $store = false, mixed $basedate = false, bool $isImported = false): bool|string {
		...
		if ($this->removeRequestOnResponse && $entryid && $userAction && !$this->isMeetingRequest($messageprops[PR_MESSAGE_CLASS])) {
	...
	public function doDecline(bool $sendresponse, mixed $basedate = false, mixed $body = false): bool {
		...
		// Opt-in; by default the request mail stays in the inbox.
		if ($this->removeRequestOnResponse && !$this->isMeetingRequest()) {
			$this->removeRequestFromInbox($basedate);
		}
